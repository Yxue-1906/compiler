...
#ifndef SRC_PARSER_AND_H
#define SRC_PARSER_AND_H

#include "../PCode.h"

namespace INTERPRETER {
    class AND : public PCode {
    public:
        const Type type;
    ...
    public:
        AND(std::string name1, std::string name2, std::string toStore)
                : type(Type::AND), name1(name1), name2(name2), toStore(toStore) {}

        virtual std::string
        to_string(std::shared_ptr<VarTable> varTable_p, const std::vector<int> &DataStack) const override {
            std::string toReturn = "AND ";
            int addr;
